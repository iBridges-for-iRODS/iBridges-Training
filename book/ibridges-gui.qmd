...
Since we have now a configuration file we can login to the iRODS server. From the main menu click on **Connect** and then **Connect to iRODS**. This opens a new pop-up window with a drop-down menu containing your configurations and a password field. If you already logged in to an iRODS server the password field will already be populated and you do not need to type in your password again.
...
The Browser is the main view of the iBridges GUI. It combines a view like a file-folder table as you know it from your file explorer with all aspects of the iRODS data objects and collections. Let's walk through it.
...
The data object contaning the file is now listed in our *demo* collection, the object carries a checksum, which is the checksum of the file. We also see the status *good*, which means all replicas (here just on file) have been checked and are consistent. 

::: {.callout-tip}

## Exercise Data Uploads (10 minutes)
...
Metadata in iRODS is provided as Key-Value-Units triple. With the text fields on the right you can add them. The Key and Value are always mandatory, you can optionally also set the Units. Click **Add**.

![](img/gui_shots/add_meta.png)
...
::: {.callout-tip}
## Exercise Metadata and Searching (10 minutes)
See Exercise slide.
:::
...
With **Open Folders** you can select the download location, choose your Downloads folder. In the options we see that we can also download the metadata as json formatted file. Let us try this out and tick the box.
...
## Preview
Click on the second of the lower tabs. If you uploaded a file in ascii, i.e. plain text format like txt, csv, tsv, you will see the first characters being printed in the screen. 

## Permissions
iRODS uses Permissions to share data among users. Click on the Permission tab. 

![](img/gui_shots/permissions.png)

At the moment you have `own` rights. That means you are allowed to do anything with the data object. 
There are also other access levels: `read`, `write` and of course rights can be retracted by choosing `delete`. Note, that the `write` access level means that you can overwrite the data and metadata but you are not allowed to delete or share. Only people with access level own, can do so.
...
You will also see that the person who uploaded the data will be known as the owner of the data. However that does NOT mean, that you have the rights to do something with the data. We will see that when we explore the data policy which is installed on this iRODS server.

## Replicas

The last part that belongs to a data object is the storage. In the *Replicas* tab you can see how many copies of the file you have under this path and on which storage they lie. Here it is relatively plain, we have one storage system called `TrainingResc`.
...
> **You can have one data object which consists of one metadata blob and two files!** 
...
<div style="
  ...
  margin-bottom: 1em;
">
  <a href="./slides/exercises_gui.html" target="_blank" rel="noopener"
